...
- Stock Spot fallback: run `binance-cli spot exchange-info --symbol-status TRADING` with `terminal.output_filter` covering all requested tickers. Verify `symbol`, `baseAsset`, `quoteAsset`, active status, Spot permission, and issuer identity. A trailing `B` is only a candidate hint. Do not fetch unfiltered futures catalogs.
...
- Stock Spot fallback: run `bgc market --action instruments --category SPOT` with `terminal.output_filter` covering all requested tickers. An `r` prefix is only a candidate hint; verify the issuer identity from returned metadata.
...
- Stock Spot fallback: run `gate-cli cex spot market pairs --format json` with `terminal.output_filter` covering all requested tickers; when needed verify the base with `gate-cli cex spot market currency --currency <BASE> --format json`. Symbol affixes are hints only. Fetch the full perpetual catalog only when the exact contract cannot be derived safely.
...
- Products: Spot and perpetuals. Read `purr lighter markets --market-type <spot|perp>` with `terminal.output_filter` covering all requested tickers. Verify a candidate with `purr lighter market --market <SYMBOL> --market-type <spot|perp>`, requiring exact identity, product, base, quote, market id, and active status. For stock Spot, issuer metadata must verify identity; ticker substrings are insufficient.
...
- Stock Spot fallback: run `okx market instruments --instType SPOT --json` with `terminal.output_filter` covering all requested tickers. An `X` prefix is only a candidate hint. Do not confuse Spot with `instCategory=3` stock-token perpetuals.
