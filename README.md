...
Stock spot buys additionally discover issuer deployments on BNB (bStocks/xStocks), Solana (xStocks), and Robinhood Chain. Crypto spot and perps never request stock swap quotes. BNB invokes the public `purr pancake swap` quote without `--execute`; the CLI owns the PancakeSwap Unified Swap API integration. Robinhood uses the platform's Uniswap V3/V4 Quoters through `FX_PLATFORM_EVM_QUOTE_URL`. Solana uses `FX_PLATFORM_DFLOW_QUOTE_URL`. Both platform endpoints use the existing `FX_PLATFORM_QUOTE_TOKEN` capability; EVM stock quotes no longer use Bitget Wallet. Gas is reserved inside the budget and the reduced input is quoted again. Solana raw xStocks amounts use the issuer multiplier; EVM balances already represent adjusted token units. Non-unit bStocks/Robinhood multipliers are excluded until their exposure semantics are supported. Missing quotes, currency rates, gas and insufficient depth remain explicit `gaps`. USDG/USD falls back to a live Kraken quote validated with the existing candle FX rules and pair catalog. No wallet access, approvals, transactions or account mutations occur.

Onchain ranked rows also retain `route`, `coverage`, `effectivePrice`, `gas`, `quoteType`, and available `feeEstimateSource`/`feeNote` fields. BNB gas uses the CLI's normalized USD estimate; Robinhood gas uses a Quoter estimate plus router overhead. Neither is a wallet simulation; approval costs and Robinhood L2 data fees are excluded. BNB coverage is the official aggregator's returned routes, which may split across pools and DEXes. Robinhood research compares direct V3 pools at standard fee tiers and V4 pools in the platform stock catalog, without historical pool scans or multi-hop search. Missing routes do not establish that a token has no market. Requote through the wallet execution workflow before making a trade proposal.
