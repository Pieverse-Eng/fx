...
This fork includes a [market-discovery skill](skills/market-discovery/SKILL.md) for Aster perpetuals and Binance Spot and USDⓈ-M perpetuals. The Aster Python 3 helper accepts multiple base tickers and an optional quote asset. The Binance helper uses Bash 4+, `binance-cli`, `jq`, and GNU `timeout` to fetch catalogs concurrently, including bStocks asset mappings, and return matching markets for one base ticker. Binance defaults to USDT; use `--quote USDC` for a specific quote or `--quote ALL` for all quotes. fx discovers the skill when running in this checkout; for another workspace, copy the complete `skills/market-discovery` directory into that workspace's `skills/` directory, or into `~/.fx/skills/` when HOME is the workspace. It is not embedded in the fx binary.
