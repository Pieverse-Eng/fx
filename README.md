...
`search_tokens` searches Bitget Wallet's public token catalog for onchain memecoins and long-tail tokens; stocks, stock-linked tokens, and major cryptocurrencies are out of scope. It accepts `query` and optional `chain`, with no configurable result limit. Explicit chains search only that chain. Without a chain, it searches the global catalog and supplements BNB Chain (`bnb`), Solana (`sol`), and Robinhood (`robinhood`) because the global search can omit those matches. Each request retrieves up to 20 candidates with `order_by: liquidity`; local selection prioritizes exact contract, name or ticker matches over fuzzy results and picks the greatest finite nonnegative reported liquidity within that match class. Ties retain retrieval order. This is a bounded candidate comparison, not exhaustive global coverage or identity verification. Missing liquidity is not treated as zero; if the best match class has no usable liquidity, the tool reports an error. Any failed lookup prevents a partial ranking from being presented as complete. The response still contains at most one token in `results`, with `name`, `symbol`, `chain`, `contract`, `twitter`, `website`, and `telegram`. Social links are provider metadata; absent or invalid links become `null`. Successful empty searches return `{"results":[]}`. This read-only tool requires curl, no wallet credentials or skill installation.
