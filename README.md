...
This fork includes a [market-discovery skill](skills/market-discovery/SKILL.md) for Aster perpetuals, Binance Spot and USDⓈ-M perpetuals, and OKX CEX Spot and linear perpetuals. The Aster Python 3 helper accepts multiple base tickers and an optional quote asset. Binance and OKX helpers use Bash 4+, the installed venue CLI, `jq`, and GNU `timeout` to run independent discovery queries concurrently and return matching markets for one base ticker. Binance joins bStocks asset mappings to Spot pairs; OKX supplements base-currency filters with category-3 stock token Spot metadata and verifies exact instrument specifications. Both default to USDT; use `--quote USDC` for a specific quote or `--quote ALL` for all quotes. fx discovers the skill when running in this checkout; for another workspace, copy the complete `skills/market-discovery` directory into that workspace's `skills/` directory, or into `~/.fx/skills/` when HOME is the workspace. It is not embedded in the fx binary.
