...
This fork includes a [market-discovery skill](skills/market-discovery/SKILL.md) for Aster perpetuals, Binance Spot and USDⓈ-M perpetuals, OKX CEX Spot and linear perpetuals, Hyperliquid Spot/native/HIP-3 perpetuals, Lighter Spot/perpetuals, Bitget Spot and USDT/USDC perpetuals, Gate Spot and USDT perpetuals, and Kraken Spot/xStocks and perpetuals. The Aster Python 3 helper accepts multiple base tickers and an optional quote asset. Other helpers use Bash 4+, the installed venue CLI, `jq`, and GNU `timeout` to run independent discovery queries concurrently and return matching markets for one base ticker. Binance joins bStocks asset mappings to Spot pairs and supports denomination-prefixed candidates; OKX supplements base-currency filters with category-3 stock token Spot metadata and verifies exact instrument specifications. Both default to USDT. Hyperliquid defaults to USDC, filters Spot by quote and perpetuals by collateral currency, preserves Spot pair IDs, and uses public Info requests via `curl` to recover full perpetual coverage when CLI search reaches its ten-result cap. Lighter fetches one complete public catalog with `purr`, filters active markets locally, defaults to USDC, and preserves market IDs and order specifications. Bitget and Gate default to USDT; Kraken defaults to USD. These three reuse their public CLI catalogs and share shell query/error handling in `catalog-common.sh`; venue matching remains in each helper. Kraken preserves post-only markets and joins perpetual specifications to current market status. Use `--quote <CURRENCY>` for a specific currency or `--quote ALL` to remove the currency filter.

fx discovers the skill when running in this checkout; for another workspace, copy the complete `skills/market-discovery` directory into that workspace's `skills/` directory, or into `~/.fx/skills/` when HOME is the workspace. It is not embedded in the fx binary.
