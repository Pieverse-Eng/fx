...
Reference selection includes Hyperliquid's default perpetual market alongside named DEXes. Bitget uses numeric platform turnover when available, including zero, and otherwise falls back to reported turnover. Gate futures trade times use fractional seconds and are converted to milliseconds before freshness checks; spot millisecond timestamps are preserved.
