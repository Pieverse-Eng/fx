...
The embedded Pieverse market-research prompt uses `discover_markets` for basket discovery, for example `{"tickers":["NVDA","TSLA"],"product":"all","quote":null}`. The tool queries eight venues concurrently and returns exact symbols, products, restrictions, market IDs, instrument specifications, and source coverage. Seven adapters use installed public venue CLIs; Hyperliquid reads its public Info API directly, including all perpetual DEXs and Spot pairs. No extra purr command is required. Each source has a 25-second deadline; failed or unsupported sources are reported rather than treated as missing listings. Product filters are `spot`, `future`, and `all`; an optional quote filter narrows results. Discovery does not fetch candles or rank execution costs.
