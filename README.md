...
This fork includes a [market-discovery skill](skills/market-discovery/SKILL.md) for Aster perpetuals, Binance Spot and USDⓈ-M perpetuals, OKX CEX Spot and linear perpetuals, and Hyperliquid Spot/native/HIP-3 perpetuals. The Aster Python 3 helper accepts multiple base tickers and an optional quote asset. Other helpers use Bash 4+, the installed venue CLI, `jq`, and GNU `timeout` to run independent discovery queries concurrently and return matching markets for one base ticker. Binance joins bStocks asset mappings to Spot pairs and supports denomination-prefixed candidates; OKX supplements base-currency filters with category-3 stock token Spot metadata and verifies exact instrument specifications. Both default to USDT. Hyperliquid defaults to USDC, filters Spot by quote and perpetuals by collateral currency, preserves Spot pair IDs, and uses public Info requests via `curl` to recover full perpetual coverage when CLI search reaches its ten-result cap. Use `--quote <CURRENCY>` for a specific currency or `--quote ALL` to remove the currency filter.
