...
`compare_trade_routes` compares public taker entry quotes for one base ticker. Spot buys use `{"ticker":"CRCL","product":"spot","amount":"1000"}`; perpetuals also require `"direction":"long"` or `"short"`. Optional `currency` is USD, USDT (default), or USDC. Spot amount is the total budget including trading fees and estimated external gas; perp amount is position notional, not margin. Perps use a common underlying quantity rounded to venue lot steps internally.
