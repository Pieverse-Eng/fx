...
`search_tokens` searches Bitget Wallet's public token catalog for onchain memecoins and long-tail tokens; stocks, stock-linked tokens, and major cryptocurrencies are out of scope. `{"query":"cashcat"}` searches without a chain filter and returns only the provider's first match. Optional `chain` accepts a provider chain code (for example `bnb`, `sol`, or `robinhood`); result count is fixed at at most one, even if the provider returns more. Results return `name`, `symbol`, `chain`, `contract`, `twitter`, `website`, and `telegram` inside `results`. Social links are supplied by the provider; missing, empty, or non-string links become `null`. A successful empty search returns `{"results":[]}`; transport and provider failures are tool errors. Search ranking is not an identity verification. This read-only tool requires curl, no wallet credentials or skill installation.
