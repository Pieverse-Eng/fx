...
The embedded Pieverse market-research prompt uses `discover_markets` for basket discovery, for example `{"tickers":["NVDA","BTC/USDC"],"product":"all"}`. Inputs accept base tickers or `BASE/QUOTE` pairs, case-insensitively; `NVDA` means `NVDA/USDT`. Each pair keeps its own quote asset, and only matching quotes are returned. The tool queries eight venues concurrently and returns exact symbols, products, restrictions, market IDs, instrument specifications, and source coverage. Seven adapters use installed public venue CLIs; Hyperliquid reads its public Info API directly, including all perpetual DEXs and Spot pairs. No extra purr command is required. Each source has a 25-second deadline; failed or unsupported sources are reported rather than treated as missing listings. Product filters are `spot`, `future`, and `all`. Discovery does not fetch candles or rank execution costs.

The market-research prompt also exposes deterministic cost tools. `calculate_venue_costs` ranks verified centralized listings for an exact taker notional. For verified stock Spot buys, `quote_onchain_stock` resolves issuer-authoritative bStocks, xStocks, and Robinhood Stock Token deployments, obtains public exact-input quotes on supported chains, normalizes each token to underlying-share exposure, and ranks routes including externally paid gas. Both tools are read-only; wallet readiness, confirmation, signing, and execution remain the embedding host's responsibility.
