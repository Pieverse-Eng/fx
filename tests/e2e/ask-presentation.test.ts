...
describe("fx ask presentation", () => {
  ...
  test("no-save advertises market discovery and rejects executable ticker input", async () => {
    const root = createRoot();
    const gateway = startFakeGateway([
    ...
    gateways.push(gateway);
    const result = await runFx(
      ["ask", "--json", "--auto", "--no-save", "Find BTC markets."],
      { cwd: root.workspace, env: gatewayEnv(root.home, gateway), timeoutMs: TIMEOUT },
    );
    expect(result.code).toBe(0);
    expect(JSON.parse(result.stdout).tool_calls).toMatchObject([
      { name: "discover_markets", status: "error" },
    ]);
    expect(existsSync(join(root.workspace, "discovery-injected"))).toBe(false);
    expect(gateway.requests).toHaveLength(2);
    expect(gateway.requests[1]!.body).toContain("InvalidTicker");
  ...
  test("market discovery aggregates concurrent CLI catalogs and reports unavailable HTTP sources", async () => {
    const root = createRoot();
    const fixtures = JSON.parse(readFileSync(join(import.meta.dirname,
      "../../src/tools/market/discovery/fixtures/stocks.json"), "utf8"));
    const fixtureDir = join(root.root, "fixtures");
    const bin = join(root.root, "bin");
    mkdirSync(fixtureDir);
    mkdirSync(bin);
    for (const fixture of fixtures) {
      writeFileSync(join(fixtureDir, `${fixture.source}.json`), JSON.stringify(fixture.data));
    }
    writeFileSync(join(fixtureDir, "kraken_spot.json"), "{}");
    const commands: Record<string, Record<string, string>> = {
      python3: { "/usr/local/lib/fx-market-data/aster_api.py exchange-info": "aster" },
      "binance-cli": {
        "spot exchange-info --symbol-status TRADING --show-permission-sets false": "binance_spot",
        "futures-usds exchange-information": "binance_future",
        "request GET https://www.binance.com/bapi/asset/v2/public/asset/asset/get-all-asset": "binance_assets",
      },
      bgc: {
        "market --action instruments --category SPOT": "bitget_spot",
        "market --action instruments --category USDT-FUTURES": "bitget_usdt",
        "market --action instruments --category USDC-FUTURES": "bitget_usdc",
      },
      "gate-cli": {
        "cex spot market pairs --format json": "gate_spot",
        "cex futures market contracts --settle usdt --format json": "gate_future",
      },
      kraken: {
        "pairs -o json": "kraken_spot",
        "assets --asset-class tokenized_asset -o json": "kraken_assets",
        "pairs --asset-class tokenized_asset -o json": "kraken_xstocks",
        "futures instruments -o json": "kraken_future",
      },
      purr: { "lighter markets --market-type all": "lighter" },
      okx: {
        "market instruments --instType SPOT --json": "okx_spot",
        "market instruments --instType SWAP --json": "okx_future",
      },
    };
    const events = join(root.root, "events");
    for (const [name, routes] of Object.entries(commands)) {
      const branches = Object.entries(routes).map(([args, source]) =>
        `${shellQuote(args)}) printf '%s\\n' '+${source}' >> ${shellQuote(events)}; sleep 0.1; ` +
        `cat ${shellQuote(join(fixtureDir, `${source}.json`))}; ` +
        `printf '%s\\n' '-${source}' >> ${shellQuote(events)} ;;`,
      ).join("\n");
      writeFileSync(join(bin, name), `#!/bin/sh\ncase "$*" in\n${branches}\n*) exit 64 ;;\nesac\n`, { mode: 0o755 });
    }
    // Refuse public HTTP deterministically: this test must never contact live venues.
    ...
    try {
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Find NVDA, TSLA, and AAPL markets."],
        { cwd: root.workspace, timeoutMs: TIMEOUT, env: {
          ...gatewayEnv(root.home, gateway), PATH: `${bin}:${process.env.PATH}`,
      ...
      expect(result.code).toBe(0);
      expect(JSON.parse(result.stdout).tool_calls).toMatchObject([
        { name: "discover_markets", status: "success" },
      ]);
      expect(gateway.requests).toHaveLength(2);
      const body = gateway.requests[1]!.body;
      for (const symbol of ["NVDAxUSD", "NVDAX_USDT", "NVDA_USDT", "RNVDAUSDT", "XNVDA-USDT", "NVDABUSDT"]) {
      ...
      for (const line of readFileSync(events, "utf8").trim().split("\n")) {
        if (line.startsWith("+")) active.add(line.slice(1));
        else active.delete(line.slice(1));
